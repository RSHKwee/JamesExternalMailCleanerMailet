...
class ParsedConfiguration {
  ...
  /**
   * Fetch both old (seen) and new messages from the mailserver. The default is to
   * fetch only messages the server are not marked as seen.
   */
  ...
  /**
   * Keep blacklisted messages on the remote mailserver. Normally, messages are
   * kept in the folder on the mailserver if they have been rejected
   */
  private boolean fieldLeaveBlacklisted = true;

  /**
   * Keep messages for remote recipients on the remote mailserver. Normally,
   * messages are kept in the folder on the mailserver if they have been rejected.
   */
  ...
  /**
   * Keep undeliverable messages on the remote mailserver. Normally, messages are
   * kept in the folder on the mailserver if they cannot be delivered.
   */
  private boolean fieldLeaveUndeliverable = true;

  /**
   * Mark retrieved messages on the remote mailserver as seen. Normally, messages
   * are marked as seen after they have been retrieved
   */
   ...
   */
  public ParsedConfiguration(HierarchicalConfiguration<ImmutableNode> configuration, UsersRepository localUsers,
   ...
   * the configuration options must not require folder updates.
   * 
   * @return boolean
   */
  protected boolean isOpenReadOnly() {
    return isLeave() && !isMarkSeen() && isLeaveBlacklisted() && !isMarkBlacklistedSeen() && isLeaveRemoteRecipient()
        && !isMarkRemoteRecipientSeen() && isLeaveUserUndefined() && !isMarkUserUndefinedSeen()
   ...
   * 
   * @param leaveRemoteReceivedHeaderInvalid The leaveRemoteReceivedHeaderInvalid
   *                                         to set
   */
   ...
   * 
   * @param markRemoteReceivedHeaderInvalidSeen The
   ...
   * 
   * @param rejectRemoteReceivedHeaderInvalid The
   *                                          rejectRemoteReceivedHeaderInvalid to
   *                                          set
   */
