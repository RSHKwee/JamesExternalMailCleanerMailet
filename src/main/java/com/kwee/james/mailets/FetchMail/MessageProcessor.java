...
package com.kwee.james.mailets.FetchMail;
 ...
 * <dt>org.apache.james.fetchmail.isMaxMessageSizeExceeded
 * (java.lang.String)</dt>
 ...
 * <li>If the Envelope contains a for: stanza, use the recipient in the
 * stanza</li>
...
public class MessageProcessor extends ProcessorAbstract {
   ...
   */
  @Override
  public void process() throws MessagingException, MailQueueException {
    ...
    if (isRecipientNotFound()) {
      if (isDeferRecipientNotFound()) {
        ...
        if (!getDeferredRecipientNotFoundMessageIDs().contains(messageID)) {
          ...
          LOGGER.debug(
              "Deferred processing of message for which the intended recipient could not be found. Message ID: {}",
              messageID);
          return;
        } else {
          getDeferredRecipientNotFoundMessageIDs().remove(messageID);
          LOGGER.debug(
              "Processing deferred message for which the intended recipient could not be found. Message ID: {}",
              messageID);
        }
      }
      ...
      StringBuilder messageBuffer = new StringBuilder(
          "Intended recipient not found. Using configured recipient as new envelope recipient - ");
  ...
  protected void rejectRecipientNotFound() throws MessagingException {
    ...
    StringBuilder messageBuffer = new StringBuilder(
        "Rejected mail for which a sole intended recipient could not be found.");
  ...
  /**
   * <p>
   * Method createMessage answers a new <code>MimeMessage</code> from the fetched
   * message.
   * </p>
   * <p/>
   * <p>
   * If the maximum message size is exceeded, an empty message is created, else
   * the new message is a copy of the received message.
   * </p>
   *
   * @return MimeMessage
   * @throws MessagingException
   ...
   * @throws MessagingException
   */
  protected Mail createMail(MimeMessage mimeMessage, MailAddress recipient)
      throws MessagingException, UnknownHostException {
    MailImpl.Builder builder = MailImpl.builder().name(MailImpl.getId()).sender(getSender()).addRecipient(recipient)
  ...
  /**
   * <p>
   * Method getSender answers a <code>MailAddress</code> for the sender. When the
   * sender local part and/or domain part can not be obtained from the mail,
   * default values are used. The flags 'defaultSenderLocalPart' and
  ...
  /**
   * <p>
   * Method computeRemoteDomain answers a <code>String</code> that is the RFC2822
   * compliant "Received : from" domain extracted from the message being processed
   * for the remote domain that sent the message.
   * </p>
   * <p/>
   * <p>
   * Often the remote domain is the domain that sent the message to the host of
   * the message store, the second "received" header, which has an index of 1.
   * Other times, messages may be received by a edge mail server and relayed
   * internally through one or more internal mail servers prior to arriving at the
   * message store host. In these cases the index is 1 + the number of internal
   * servers through which a mail passes.
   * </p>
   * <p>
   * The index of the header to use is specified by the configuration parameter
   * <code>RemoteReceivedHeaderIndex</code>. This is set to point to the received
   * header prior to the remote mail server, the one prior to the edge mail
   * server.
   * </p>
   * <p>
   * "received" headers are searched starting at the specified index. If a domain
   * in the "received" header is not found, successively closer "received" headers
   * are tried. If a domain is not found in this way, the local machine is used as
   * the domain. Finally, if the local domain cannot be determined, the local
  ...
  protected String computeRemoteDomain() throws MessagingException {
    ...
    if (hasHeaders) {
      ...
      for (int headerIndex = headers.length > getRemoteReceivedHeaderIndex() ? getRemoteReceivedHeaderIndex()
          : headers.length - 1; headerIndex >= 0 && domainBuffer.length() == 0; headerIndex--) {
        // Find the "from" token
  ...
  protected boolean isLocalRecipient(MailAddress recipient) throws UsersRepositoryException {
    ...
    // See JAMES-1135
    return isLocalServer(recipient) && getLocalUsers().contains(getLocalUsers().getUsername(recipient));
  }
   ...
   * @throws MailQueueException
   */
  protected void sendMail(Mail mail) throws MessagingException, MailQueueException {
    // queue the mail
  ...
  protected MailAddress getIntendedRecipient() throws MessagingException {
    ...
    if (isIgnoreRecipientHeader()) {
      StringBuilder messageBuffer = new StringBuilder(
          "Ignoring recipient header. Using configured recipient as new envelope recipient: ");
    ...
    if (allRecipients.length == 1) {
      ...
      StringBuilder messageBuffer = new StringBuilder(
          "Using sole recipient header address as new envelope recipient: ");
      messageBuffer.append(recipient);
      messageBuffer.append('.');
      logStatusInfo(messageBuffer.toString());
      return recipient;
  ...
  /**
   * Returns <code>boolean</code> indicating if the message to be delivered was
   * unprocessed in a previous delivery attempt.
   *
   * @return boolean
   */
  ...
  protected void addMailAttributes(Mail aMail) throws MessagingException {
    ...
    aMail.setAttribute(
        new Attribute(makeAttributeName("folderName"), AttributeValue.of(getMessageIn().getFolder().getFullName())));
    ...
    if (isMaxMessageSizeExceeded()) {
      aMail.setAttribute(new Attribute(makeAttributeName("isMaxMessageSizeExceeded"),
          AttributeValue.of(Integer.toString(getMessageIn().getSize()))));
    }
  ...
  protected void addErrorMessages(Mail mail) throws MessagingException {
    if (isMaxMessageSizeExceeded()) {
      StringBuilder msgBuffer = new StringBuilder(
          "550 - Rejected - This message has been rejected as the message size of ");
